...
public class MemoController {
  ...
  public ResponseEntity updateMemo(@Validated @RequestBody MemoUpdateRequest memoUpdateRequest, @Auth Long memberId){
    SavedId savedId = new SavedId(memoService.update(memoUpdateRequest,memberId));
    return ResponseEntity.ok(new BaseResponse(savedId,HttpStatus.OK.value(),CustomCode.SUCCESS_UPDATE));
  }
  @DeleteMapping("/memo")//메모 삭제 API
  ...
  public ResponseEntity deleteMemo(@Validated @RequestBody MemoDeleteRequest memoDeleteRequest, @Auth Long memberId){
    memoService.delete(memoDeleteRequest,memberId);
